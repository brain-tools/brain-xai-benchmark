...
from torch.nn import AdaptiveAvgPool2d, AdaptiveAvgPool3d, Sequential
...
sys.path.insert(
    0,
    "/sc-projects/sc-proj-cc15-cn-ukbiobank/analyses/Explanation-benchmark-paper"
)
from model_training.models.densenet.densenet import DenseNet as DenseNet3D
...
class DenseNetAdaptiveAvgPool3DCanonizer(AttributeCanonizer):
    """
    Canonizer specifically for the 3D DenseNet implementation.

    Rewrites the model's forward pass so that:
      - features -> final_relu -> AdaptiveAvgPool3d((1,1,1))
      - output is flattened and passed through classifier

    This mirrors torchvision's 2D canonizer but adapted for 3D.
    """
    ...
    @classmethod
    def _attribute_map(cls, name, module):
        # Correct instance check for your custom 3D DenseNet
        if isinstance(module, DenseNet3D):
            return {
                "forward": cls.forward.__get__(module),
            }
        return None
    ...
    def copy(self):
        return DenseNetAdaptiveAvgPool3DCanonizer()
    ...
    def register(self, module, attributes):
        ...
        Insert:
            final_relu
            adaptive_avg_pool3d
        at the end of module.features
        """

        module.features.add_module("final_relu", ReLU(inplace=True))
        module.features.add_module("adaptive_avg_pool3d", AdaptiveAvgPool3d((1, 1, 1)))

        super().register(module, attributes)

    def remove(self):
        """
        Remove the two modules added above.
        Restore the original forward function.
        """
        # Drop final two elements: final_relu and adaptive_avg_pool3d
        self.module.features = Sequential(*list(self.module.features.children())[:-2])
    ...
    def forward(self, x):
        """
        New forward pass used during canonization.
...
class DefaultDenseNetCanonizer3D(CorrectCompositeCanonizer):
    def __init__(self):
        super().__init__((
            DenseNetAdaptiveAvgPool3DCanonizer(),
        ))
